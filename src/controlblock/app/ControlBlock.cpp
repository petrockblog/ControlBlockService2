...
ControlBlock::ControlBlock(IUInputFactory &uiFactoryRef,
  ...
  if (configRef.getConfiguration(0).isEnabled() && configRef.getConfiguration(0).isPowerSwitchEnabled()) {
    powerSwitch = new PowerSwitch(*digitalIO[0], PowerSwitch::ShutdownActivated::ACTIVATED);
  }
}
...
void ControlBlock::configureDevice(IDigitalIO *digitalIO, InputDevice::GamepadType_e type) {
  ...
  switch (type) {
    case InputDevice::GAMEPAD_ARCADE:digitalIO->configureDevice(IDigitalIO::DIO_DEVICE_ALLIN);
      break;
    case InputDevice::GAMEPAD_MAME:digitalIO->configureDevice(IDigitalIO::DIO_DEVICE_SNES);
      break;
    case InputDevice::GAMEPAD_SNES:digitalIO->configureDevice(IDigitalIO::DIO_DEVICE_SNES);
      break;
    case InputDevice::GAMEPAD_NES:digitalIO->configureDevice(IDigitalIO::DIO_DEVICE_SNES);
      break;
