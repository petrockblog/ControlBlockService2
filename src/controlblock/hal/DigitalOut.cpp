...
void DigitalOut::configureDevice(DO_Device device) {
  switch (device) {
    case DO_DEVICE_POWERSWITCH:
      // do nothing here. It is assumed that the direction is set from outside, e.g., via device tree overlay.
      break;
...
void DigitalOut::setLevel(DO_Channel_e channel, DO_Level_e level, BoardNumber_e board) {
  ...
  switch (channel) {
    case DO_CHANNEL_TOPOWERSWITCH:powerSwitchOut_port_->Write(outlevel != MCP23S17PI::LEVEL_LOW);
      break;
